#!/usr/bin/env python3
"""Post-source DANGER3 handoff for priority-1 divisor/additive wins.

The priority-1 source packet now has four source-stage yes answers.  This
gate attaches those yes answers to the already-owned DANGER3 framing,
same-j bridge, X_1(16), halving, and official-vpp ladder.
"""
...
class Priority1PostSourceDANGER3Row:
    name: str
    source_lane: str
...
@dataclass(frozen=True)
class Priority1PostSourceDANGER3Handoff:
    dependency_markers_present: int
    dependency_markers_total: int
    source_answer_router_ok: bool
    rows: tuple[Priority1PostSourceDANGER3Row, ...]
    row_count: int
    priority1_source_yes_rows: int
...
def row_from_source_answer(answer: Priority1SourceAnswerRow) -> Priority1PostSourceDANGER3Row:
    decision = classify_candidate(source_candidate(f"{answer.source_query_name}_post_source"))
    return Priority1PostSourceDANGER3Row(
        name=f"post_source_{answer.source_query_name}",
        source_lane=answer.source_query_name,
...
def downstream_rows() -> tuple[Priority1PostSourceDANGER3Row, ...]:
    cases = (
        (
    ...
    for name, stage, kwargs, expected, submission_boundary in cases:
        ...
        rows.append(
            Priority1PostSourceDANGER3Row(
                name=name,
                source_lane="generic_post_source_ladder",
                source_answer_name="not_fixture_specific",
                route_stage=stage,
                framing_decision=decision,
...
def profile_priority1_post_source_danger3_handoff() -> Priority1PostSourceDANGER3Handoff:
    markers_present = sum(marker_present(path, marker) for path, marker in DEPENDENCY_MARKERS)
    source_router = profile_priority1_source_answer_router()
    source_yes_rows = tuple(
        row_from_source_answer(answer)
        for answer in source_router.rows
    ...
    row_ok = (
        ...
        and source_router.row_ok
        and len(source_yes_rows) == 4
        and len(rows) == 10
        and sum(row.source_stage_yes_answer for row in rows) == 4
        and current_source == 0
        and source_stage_closed == 9
        and policy_missing == 4
    ...
    return Priority1PostSourceDANGER3Handoff(
        dependency_markers_present=markers_present,
        dependency_markers_total=len(DEPENDENCY_MARKERS),
        source_answer_router_ok=source_router.row_ok,
        rows=rows,
        row_count=len(rows),
        priority1_source_yes_rows=sum(row.source_stage_yes_answer for row in rows),
...
def main() -> int:
    profile = profile_priority1_post_source_danger3_handoff()
    print("p25 KSY-y priority-1 post-source DANGER3 handoff gate")
    print("dependencies")
    ...
    print(f"  source_answer_router_ok={int(profile.source_answer_router_ok)}")
    print("handoff_rows")
    for row in profile.rows:
        decision = row.framing_decision
        print(
            "  "
            f"{row.name}: lane={row.source_lane} stage={row.route_stage} "
    ...
    print(f"  priority1_source_yes_rows={profile.priority1_source_yes_rows}")
    ...
    print("  priority1_source_yes_is_not_submission_ready=1")
    ...
    print(
        "ksy_y_priority1_post_source_danger3_handoff_rows="
        f"{int(profile.row_ok)}/1"
    )
    if not profile.row_ok:
        raise SystemExit("priority-1 post-source DANGER3 handoff regression failed")
    return 0
