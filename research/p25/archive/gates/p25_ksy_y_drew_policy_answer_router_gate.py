#!/usr/bin/env python3
"""Answer router for Drew-facing finite-identity policy replies."""
...
from p25_ksy_y_drew_policy_question_packet_gate import (
    profile_drew_policy_question_packet,
)
...
DEPENDENCY_MARKERS = (
    (
        RESEARCH / "p25_ksy_y_drew_policy_question_packet_20260614.md",
...
def answer_rows() -> tuple[DrewPolicyAnswerRow, ...]:
    return (
        answer_row(
        ...
        answer_row(
            name="external_policy_yes_for_ksy_h90",
            family="continue",
            answer_shape="Drew gives policy yes for p-specialized KSY/Kubert-Lang/H90 identity",
            expected_decision="danger3_unblocked_cross_level_bridge_missing",
            recommendation="continue to same-j X_1(8112) bridge construction",
            route_state="DANGER3 framing unblocked; bridge missing",
...
def profile_drew_policy_answer_router() -> DrewPolicyAnswerRouter:
    markers_present = sum(marker_present(path, marker) for path, marker in DEPENDENCY_MARKERS)
    question_packet = profile_drew_policy_question_packet()
    rows = answer_rows()
...
def main() -> int:
    profile = profile_drew_policy_answer_router()
    print("p25 KSY-y Drew policy answer router gate")
    print("dependencies")
    ...
    print(f"ksy_y_drew_policy_answer_router_rows={int(profile.row_ok)}/1")
    if not profile.row_ok:
        raise SystemExit("Drew policy answer router regression failed")
    return 0
