...
First, does the current p24 challenge rule out exploiting CM in the same sense
as the no-CM wording copied in this fork's top-level README, or is that not a
binding constraint for the current `DANGER3` p24 target?

If CM is admissible as a proof/certificate strategy: for the p24 CM field
below, is there a known explicit CM / Shimura reciprocity / elliptic-unit /
Lang construction that selects one embedded unramified `157/211` Hilbert-class
phase, or an obstruction showing that such a class-set-free selector cannot
exist without carrying a degree-about-`3107441` recovery object?
...
The latest Jacobi-sum literature check splits the gap more sharply.
Kubert-Lichtenbaum/Weil Jacobi-sum Hecke characters and
Langlands/Hasse-Davenport identities explain the reduced packet and the
degree-`89` `R_179`/kernel-polynomial anchor:

```text
p mod 179 = 77
ord_179(p) = 89 = (179-1)/2
nonzero 179-cyclotomic exponents split into two Frobenius half-orbits
inversion swaps the half-orbits
```

But ordinary visible cyclotomic Frobenius still has the wrong order:

```text
ord_{7*179}(p) = 89
actual post-B/C rho quotient order = 7*179 = 1253
```

Anderson/Taniyama-group language removes the worry that Jacobi-sum Hecke
characters must live over an abelian base, but it does not by itself select
the p24 unramified quotient.  The remaining bridge is specifically a
CM-Artin pullback of the reduced Jacobi packet along the unramified
`rho=p^780` quotient, modulo a verifier-invisible pure `C_179` residual.

## Current Non-Enumerative Selector Candidate

The main plausible selector route besides a direct CM/Lang producer is a
low-moment phase selector.
...
There is also a related scalar/content route that may be easier to ask about.
Define the tower-native axis coefficient space
...
For each nontrivial Frobenius packet factor `f_a | Phi_3107441`, the refined
target is injectivity of

```text
T_a : W_axis -> F_p[X]/(f_a),    T_a(w)=sum_r w(r)F_r(X).
```

Since `deg f_a = ord_n(p) = 388430`, the finite rank surface is well below
`sqrt(p)` even across all eight packet orbits:

```text
8 * 368 * 388430 = 1143537920 < 10^12.
```

Small actual-CM stress tests are clean (`191/191` eligible packets and
`630/630` all-origin eligible packets injective in the latest bounded runs),
but this is still a selected-prime p-unit / axis-injectivity theorem, not a
proof.
...
5. Does the `W_axis` injectivity target above look like a known normal-basis,
   resolvent, or explicit-CM nonvanishing theorem, or is selected-origin
   hyperplane avoidance genuinely as hard as the original phase selection?

6. Are there references or standard tools we should be using here, especially
