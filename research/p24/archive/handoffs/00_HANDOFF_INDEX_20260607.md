...
We do not have the final p24 certificate yet. We do have much narrower
mathematical targets than we started with.

Admissibility warning: the current `AndrewVSutherland/DANGER3` README and the
local `p24/upstream_DANGER3/README.md` do not visibly contain a no-CM rule,
but this fork's top-level `README.md` does say the challenge is to find triples
"without exploiting supersingular curves or CM."  Until that is clarified,
the CM/Lang/Jacobi work is best labeled a conditional/diagnostic route.  It
may still expose a finite-field identity, but it should not be represented as
strictly challenge-final if the no-CM wording is binding.

Strict/no-CM status as of the latest refresh: no asymptotic strict-DANGER
speedup is known.  The trace-residue oracle is information-rich but prices
back to `Theta(sqrt(p))`: pure `2^40` remains the best sampled exact-residue
level with `gamma0/sqrt = 1.649267`, and mixed odd CRT residues do not improve
the exponent.  Small exact Montgomery trace-bucket spectra again show no
stable low-frequency or small-residue selector.  Thus the strict-admissible
path needs a genuinely new finite-field identity, not another rejection
filter.

The main conditional producer theorem is:
...
The best auxiliary selectors now split into two sub-sqrt theorem microscopes:

```text
low-moment selector:
  30 selector constraints, 28 genuinely new selected relative traces,
  or 8172 parent-field coefficients if kept as functions

L1 / W_axis content selector:
  dim W_axis = 368,
  all eight p24 packet rank surface = 1143537920 < 10^12 = sqrt_floor(p)
```

Latest 2026-06-08 refinement: the L1/W_axis target is best stated as a
one-factor Moore/normality theorem.  After adjoining `E=F_p(mu_m)`, one
degree-`388430` packet splits into `70` tensor factors `B_i/E` of degree
`5549`; since `368 < 5549`, it is enough to prove that the 368 selected
K-character resolvents have nonzero Moore determinant in one `B_i`:

```text
det(R_s^(Q^j)) != 0,  s in S_axis, 0 <= j < 368,  Q=|E|.
```

A fresh holdout run found `25` one-factor Moore rows, `0`
coordinate/Moore mismatches, `2` dimension-possible rows, and `0`
dimension-possible Moore failures.  The matching block scan found `0`
dimension-possible block, pair, or full-directness failures.  Lean now names
the bridge as `base_injective_from_tensor_factor_moore_coordinates` in
`p24/lean/AxisInjectivityGate.lean`: Moore-coordinate injectivity in one
tensor factor implies base packet axis injectivity and hence the downstream
L1/content gate.

For proof/descent, the sharper surface is the denominator-safe leading
trace-frame coordinate:
...
The current compression theorem to prove is: construct a representative
degree-8 leading determinant-line section over `K_m Q(zeta_n)^<p>` whose norm
is a p-unit at the selected prime, and prove determinant-line p-unit transport
across the 70 scalar-extension tensor factors.  A fresh selected-tail
equivariance holdout found `14` rows in `6` row-groups with uniform full and
tail zero/nonzero status across tensor factors (`selected_tail_transport_survives=1`
in every group).  Raw tail norms often differed, so the theorem must use
determinant-line transport rather than equality of ad hoc residual-tail
determinants.

Lean now also pins the p24 norm-compressed accounting in
`p24/lean/TraceFrameNormCompressedCertificateGate.lean`: `560=8*70`,
`179+179+10=368`, the single-leading all-H-packet surface
`5915320320 < 10^12`, the all-70-factor single-leading surface
`414072422400 < 10^12`, the factorized all-70-factor surface
`333779846400 < 10^12`, and the norm-compressed payload
`10920 < 10^12`.  The gate also now has the verifier-side implication
`N_lead * U_lead = 1 => N_lead != 0 => all beta orbits good`, abstracted as
`all_beta_orbits_good_from_degree8_inverse_payload`.

Pinned cyclic-resultant/local-unit audit refresh:
`trace_frame_beta_product_resultant_audit.py --only-D -10919 --only-m 12
--target constant_plus_4 --target constant_plus_3` returned `8` rows with
`resultant_match=1`, `resultant_in_E=1`, `resultant_zero=0`,
`coeff_semilin_fail=0`, `trace_recon_fail=0`, and `orbit_zero_products=0`.
This confirms the finite crossed-product packaging in the compact row, while
again ruling out the ordinary constant-value orbit shortcut.

Targeted literature check: Gross-Zagier/Borcherds-style CM-value
factorization and local Borcherds products are relevant only after
`Xi_lead` is constructed as a p-integral modular/Borcherds/Fitting object
with known divisor.  The precise missing step is to identify the current
leading determinant line with such an object, then prove the selected p24 CM
prime has zero local intersection with its divisor.  Without that
identification, the known factorization theorems do not directly apply.

The verifier surface is small and the finite implication chain is strong. The
remaining risk is the arithmetic producer theorem and anti-collision /
nonvanishing theorem, not the downstream checker.
...
unramified `157/211` phase production, a low-moment relative-trace producer,
or a `W_axis` p-unit/nonvanishing theorem.  If doing local proof work rather
than external synthesis, read
...
low-moment producer microscope:
  30 selector constraints, 28 new selected values
  8172 parent-field coefficients = 8.172e-9 * sqrt(p)

L1 / W_axis content microscope:
  dim W_axis = 368
  deg f_a = ord_n(p) = 388430
  all packets: 8 * 368 * 388430 = 1143537920 < 10^12
  sharper theorem: after tensoring with E=F_p(mu_m), one degree-5549
    factor B_i/E suffices; 368 < 5549
```
...
actual-CM sweep, bounded larger 2026-06-08 spot rerun:
  rows=134
...
sparse-relation dictionary:
  ...
  L1 / W_axis scalar-content target:
    W_axis = {a0 + g_2(r mod 2) + g_157(r mod 157) + g_211(r mod 211)}
    dim W_axis = 368
    target: T_a : W_axis -> F_p[X]/(f_a) is injective for each p24 packet
    all-packet rank surface: 8 * 368 * 388430 = 1143537920 < sqrt(p)
    wider eligible bounded scan: 191/191 injective, no L1 zeros
    all-origin eligible bounded scan: 630/630 injective, no L1 zeros
    rank-safe boundary falsifier refresh: 386/386 eligible rows injective,
      full-K injective, no L1 zeros, no block/pair/cross failures
    Lean annihilator refinement:
      AxisInjectivityGate now proves axis injective iff annihilator-avoidance
        under the full-evaluation-kernel definition of the annihilator
      p24 packet-field module arithmetic pinned as 1+1+2*78+210=368
      equivalent proof target: gcd(A_a(Z), Phi_1 Phi_2 Phi_157 Phi_211)=1
      semisimple component criterion: injectivity follows from cancellable
        scaling in each axis component, i.e. nonzero packet projections in
        214 packet-field components
    tensor-factor reduction: A_a tensor F_p(mu_m) splits into 70 factors
      of degree 5549 over E; one factor injective descends to T_a injective
    pinned dual-basis/CRT refresh on D=-10919:
      direct m=3,4,12 dual-basis rows all have trace-window mismatches = 0
      direct m=3,4,12 max-rank profile failures = 0
      m=12 CRT marginal rank/capacity failures = 0
      m=12 constantplus4plus3 rank = 6/6
      m=12 twisted trace-frame recovers full axis rank by short frames
    operational note: direct/pinned tensor rows are productive; blind
      composite scanning is too slow for the information gained
```
...
Live sqrt-scale baseline as of 2026-06-08: `pomerance.c` has a new p24
descending odd-part ladder branch for
`909494701773 = 2*454747350887 - 1` and
`454747350887 = 2*227373675443 + 1`.  The branch was checked by an
independent Python x-only ladder identity test over the actual p24 field.
The replacement run is
`runs/p24_generic_ladderfix_1p65sqrt_20260608_111143`; `/tmp/p24_run_dir.txt`
points there.  It has 10 workers, aggregate budget
`1.65e12 = 1.65*sqrt(p)`, and a watcher that writes `HIT_DETECTED.txt` then
SIGTERMs workers on `Verified: PASS`.  Early aggregate rate was about
`1.27M/s`, roughly 1.7x the stopped warm-up tranche.  This is still a
sqrt-scale lottery and does not change the missing asymptotic theorem.
