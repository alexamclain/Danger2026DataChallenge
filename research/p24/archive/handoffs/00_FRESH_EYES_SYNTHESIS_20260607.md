...
2026-06-08 scalar/content companion route:
  W_axis = {a0 + g_2(r mod 2) + g_157(r mod 157) + g_211(r mod 211)}
  has dimension 368; for each p24 packet factor f_a of degree 388430, the
  target map T_a : W_axis -> F_p[X]/(f_a) should be injective.  Across all
  eight packets this is 1143537920 coordinates, still below sqrt(p).  Latest
  bounded actual-CM controls are 191/191 and 630/630 injective, with no L1
  zeros.  This route still needs selected-prime p-unit / hyperplane avoidance.
```
...
now concentrated on the product/divisor theorem plus the low-moment / W_axis
selector theorem microscopes.
```
