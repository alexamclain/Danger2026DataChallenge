...
structure SelectedProductFormulaProducerObligations where
  selectedDefectCZeroFiber : Prop
  rawTwoLevelInversionComplement : Prop
  selectedAffineRowBalance : Prop

def SatisfiesSelectedProductFormulaProducerObligations
    (obligations : SelectedProductFormulaProducerObligations) : Prop :=
  obligations.selectedDefectCZeroFiber ∧
  obligations.rawTwoLevelInversionComplement ∧
  obligations.selectedAffineRowBalance
...
def valueSideFromSelectedProductFormulaProducer
    (obligations : SelectedProductFormulaProducerObligations) :
    ValueSideIdentities where
  cRowSumsIndependent := obligations.selectedAffineRowBalance
  cZeroFiberVanishes := obligations.selectedDefectCZeroFiber
  inversionComplementConstantOffCZero :=
    obligations.rawTwoLevelInversionComplement
...
structure KubertLichtenbaumR179OrbitAnchorObligations where
  mixedLevelJacobiHeckeCharacterExists : Prop
  langlandsHasseDavenportOrbitProduct : Prop
  frobenius179SplitsIntoTwoHalfProducts : Prop
  inversionQuotientIsDegree89KernelAnchor : Prop
  ordinaryCyclotomicFrobeniusDoesNotSelectPostBCQuotient : Prop

def SatisfiesKubertLichtenbaumR179OrbitAnchorObligations
    (obligations : KubertLichtenbaumR179OrbitAnchorObligations) :
    Prop :=
  obligations.mixedLevelJacobiHeckeCharacterExists ∧
  obligations.langlandsHasseDavenportOrbitProduct ∧
  obligations.frobenius179SplitsIntoTwoHalfProducts ∧
  obligations.inversionQuotientIsDegree89KernelAnchor ∧
  obligations.ordinaryCyclotomicFrobeniusDoesNotSelectPostBCQuotient

structure SelectedR179KernelUnitProducerObligations where
  klOrbitAnchor : KubertLichtenbaumR179OrbitAnchorObligations
  selectedPacketPulledBackAlongUnramifiedRhoQuotient : Prop
  selectedFiniteCoordinatePIntegral : Prop
  resultantOrBasepointAvoidance : Prop
  degenerateAnchorIsR179KernelUnit : Prop

def SatisfiesSelectedR179KernelUnitProducerObligations
    (obligations : SelectedR179KernelUnitProducerObligations) :
    Prop :=
  SatisfiesKubertLichtenbaumR179OrbitAnchorObligations
    obligations.klOrbitAnchor ∧
  obligations.selectedPacketPulledBackAlongUnramifiedRhoQuotient ∧
  obligations.selectedFiniteCoordinatePIntegral ∧
  obligations.resultantOrBasepointAvoidance ∧
  obligations.degenerateAnchorIsR179KernelUnit

def r179KernelUnitFromSelectedKLPullback
    (obligations : SelectedR179KernelUnitProducerObligations) : Prop :=
  obligations.degenerateAnchorIsR179KernelUnit

structure SelectedKLPuncturedHasseDavenportProducerInputs where
  selectedR179KernelUnit : SelectedR179KernelUnitProducerObligations
  puncturedNonzeroRowsGiveReducedJacobiCarry :
    ReducedJacobiCarryObligations
  pureC179ResidualValueSide :
    PureC179ResidualValueSideObligations
...
def SatisfiesSelectedKLPuncturedHasseDavenportProducerInputs
    (inputs : SelectedKLPuncturedHasseDavenportProducerInputs) :
    Prop :=
  SatisfiesSelectedR179KernelUnitProducerObligations
    inputs.selectedR179KernelUnit ∧
  SatisfiesReducedJacobiCarryObligations
    inputs.puncturedNonzeroRowsGiveReducedJacobiCarry ∧
  SatisfiesPureC179ResidualValueSideObligations
    inputs.pureC179ResidualValueSide ∧
  inputs.adjacentAnchorRowSumSliceMatched ∧
  inputs.cNontrivialResidualMatchedByR179 ∧
  inputs.selectedChildSubtractionCompatible

def puncturedHasseDavenportAnchorFromSelectedKLR179Pullback
    (inputs : SelectedKLPuncturedHasseDavenportProducerInputs) :
    PuncturedHasseDavenportAnchorProducerObligations where
  puncturedNonzeroRowsGiveReducedJacobiCarry :=
    inputs.puncturedNonzeroRowsGiveReducedJacobiCarry
  pureC179ResidualValueSide := inputs.pureC179ResidualValueSide
  degenerateAnchorIsR179KernelUnit :=
    r179KernelUnitFromSelectedKLPullback inputs.selectedR179KernelUnit
...
def AllSelectedProductFormulaProducerObligations {Character : Type}
    (obligations : Character → SelectedProductFormulaProducerObligations) :
    Prop :=
  ∀ chi, SatisfiesSelectedProductFormulaProducerObligations
    (obligations chi)
...
def AllSelectedKLPuncturedHasseDavenportProducerInputs {Character : Type}
    (inputs : Character → SelectedKLPuncturedHasseDavenportProducerInputs) :
    Prop :=
  ∀ chi, SatisfiesSelectedKLPuncturedHasseDavenportProducerInputs
    (inputs chi)
...
theorem value_side_from_selected_product_formula_producer_obligations
    {Character : Type}
    (obligations : Character → SelectedProductFormulaProducerObligations)
    (h_product :
      AllSelectedProductFormulaProducerObligations obligations) :
    AllValueSideIdentities
      (fun chi =>
        valueSideFromSelectedProductFormulaProducer (obligations chi)) := by
  intro chi
  rcases h_product chi with ⟨h_zero, h_inversion, h_row_balance⟩
  exact ⟨h_row_balance, h_zero, h_inversion⟩
...
theorem punctured_hasse_davenport_anchor_from_selected_kl_r179_pullback
    (inputs : SelectedKLPuncturedHasseDavenportProducerInputs)
    (h_inputs :
      SatisfiesSelectedKLPuncturedHasseDavenportProducerInputs inputs) :
    SatisfiesPuncturedHasseDavenportAnchorProducerObligations
      (puncturedHasseDavenportAnchorFromSelectedKLR179Pullback inputs) := by
  rcases h_inputs with
    ⟨h_r179, h_carry, h_residual, h_row_sum, h_c_residual, h_child⟩
  rcases h_r179 with
    ⟨_h_kl, _h_pullback, _h_integral, _h_avoidance, h_anchor⟩
  exact ⟨h_carry, h_residual, h_anchor, h_row_sum, h_c_residual, h_child⟩

theorem verifier_equivalent_reduced_jacobi_carry_from_selected_kl_r179_pullback
    (inputs : SelectedKLPuncturedHasseDavenportProducerInputs)
    (h_inputs :
      SatisfiesSelectedKLPuncturedHasseDavenportProducerInputs inputs) :
    SatisfiesVerifierEquivalentReducedJacobiCarryObligations
      (verifierEquivalentReducedJacobiCarryFromPuncturedHasseDavenportAnchor
        (puncturedHasseDavenportAnchorFromSelectedKLR179Pullback inputs)) := by
  exact verifier_equivalent_reduced_jacobi_carry_from_punctured_hasse_davenport_anchor
    (puncturedHasseDavenportAnchorFromSelectedKLR179Pullback inputs)
    (punctured_hasse_davenport_anchor_from_selected_kl_r179_pullback
      inputs h_inputs)
...
theorem verifier_equivalent_reduced_jacobi_carry_from_selected_kl_inputs
    {Character : Type}
    (inputs : Character → SelectedKLPuncturedHasseDavenportProducerInputs)
    (h_inputs : AllSelectedKLPuncturedHasseDavenportProducerInputs inputs) :
    AllVerifierEquivalentReducedJacobiCarryObligations
      (fun chi =>
        verifierEquivalentReducedJacobiCarryFromPuncturedHasseDavenportAnchor
          (puncturedHasseDavenportAnchorFromSelectedKLR179Pullback
            (inputs chi))) := by
  intro chi
  exact verifier_equivalent_reduced_jacobi_carry_from_selected_kl_r179_pullback
    (inputs chi) (h_inputs chi)

theorem value_side_from_selected_kl_inputs
    {Character : Type}
    (inputs : Character → SelectedKLPuncturedHasseDavenportProducerInputs)
    (h_inputs : AllSelectedKLPuncturedHasseDavenportProducerInputs inputs) :
    AllValueSideIdentities
  ...
  exact value_side_from_verifier_equivalent_reduced_jacobi_carry_obligations
    (fun chi =>
      verifierEquivalentReducedJacobiCarryFromPuncturedHasseDavenportAnchor
        (puncturedHasseDavenportAnchorFromSelectedKLR179Pullback
          (inputs chi)))
    (verifier_equivalent_reduced_jacobi_carry_from_selected_kl_inputs
      inputs h_inputs)
...
theorem four_dual_families_from_selected_product_formula_producer_obligations
    {Character : Type}
    (obligations : Character → SelectedProductFormulaProducerObligations)
    (families : Character → DualFourierFamilies)
    (h_value_to_dual :
      ∀ chi,
        SatisfiesValueSideIdentities
          (valueSideFromSelectedProductFormulaProducer
            (obligations chi)) →
        SatisfiesFourDualFamilies (families chi))
    (h_product :
      AllSelectedProductFormulaProducerObligations obligations) :
  ...
  exact four_dual_families_from_value_side_identities
    (fun chi =>
      valueSideFromSelectedProductFormulaProducer (obligations chi))
    families h_value_to_dual
    (value_side_from_selected_product_formula_producer_obligations
      obligations h_product)
...
theorem four_dual_families_from_selected_kl_inputs
    {Character : Type}
    (inputs : Character → SelectedKLPuncturedHasseDavenportProducerInputs)
    (families : Character → DualFourierFamilies)
    (h_value_to_dual :
      ...
      valueSideFromVerifierEquivalentReducedJacobiCarry
        (verifierEquivalentReducedJacobiCarryFromPuncturedHasseDavenportAnchor
          (puncturedHasseDavenportAnchorFromSelectedKLR179Pullback
            (inputs chi))))
    families h_value_to_dual
    (value_side_from_selected_kl_inputs inputs h_inputs)
...
theorem hcoset_verifier_from_selected_product_formula_producer_obligations
    {Character Row Coset : Type}
    (obligations : Character → SelectedProductFormulaProducerObligations)
    (families : Character → DualFourierFamilies)
    (inAdmissibleSpan forbiddenBidegreeZero finalTraceZero rightCoboundary :
  ...
  exact hcoset_verifier_from_value_side_identities
    ...
    rightCoboundary productCoboundary characterZero centered hcosetZero
    allRowsCentered allCharacterPayloadZero h_value_to_dual h_dual_complete
    h_admissible_support h_final h_right h_product_coboundary h_character
    h_character_collect h_hcoset
    (value_side_from_selected_product_formula_producer_obligations
      obligations h_product)
    h_centered
...
theorem hcoset_verifier_from_selected_kl_inputs
    {Character Row Coset : Type}
    (inputs : Character → SelectedKLPuncturedHasseDavenportProducerInputs)
    (families : Character → DualFourierFamilies)
    (inAdmissibleSpan forbiddenBidegreeZero finalTraceZero rightCoboundary :
