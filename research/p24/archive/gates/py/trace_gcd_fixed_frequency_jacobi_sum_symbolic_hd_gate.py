...
def orbit_under_multiplier(start: int, multiplier: int, modulus: int) -> list[int]:
    orbit: list[int] = []
    value = start % modulus
    ...
    if value != orbit[0]:
        raise RuntimeError("multiplier orbit did not close at the start")
    return orbit


def p24_kubert_lichtenbaum_orbit_anchor_checks() -> dict[str, int]:
    """Record the KL/Langlands orbit identity in the p24 R_179 shape.

    Kubert-Lichtenbaum's mixed-level Jacobi-sum Hecke characters and the
    Langlands/Hasse-Davenport Gauss-product identities explain the visible
    Jacobi packet and its Frobenius orbit products.  For p24, p has order 89
    modulo 179, so the nonzero 179-cyclotomic characters split into exactly
    two Frobenius half-orbits.  Inversion swaps those halves, and the
    real/inversion quotient has degree 89, matching the R_179 kernel
    polynomial anchor.

    This is positive progress but not the final selector: the same ordinary
    cyclotomic Frobenius is trivial on the right 7-axis and has total visible
    order 89 rather than the actual post-B/C order 7*179.  Thus this source
    supplies the degree-89 anchor shape; the remaining bridge is still the
    unramified CM-Artin pullback along the rho quotient.
    """
    p_mod_c = P24 % P24_C_DEGREE
    c_order = order_by_known_factors(
        p_mod_c, P24_C_DEGREE, P24_C_DEGREE - 1
    )
    right_order = order_by_known_factors(
        P24 % RIGHT_DEGREE, RIGHT_DEGREE, RIGHT_DEGREE - 1
    )
    level_order = order_by_known_factors(
        P24 % P24_JACOBI_LEVEL,
        P24_JACOBI_LEVEL,
        (RIGHT_DEGREE - 1) * (P24_C_DEGREE - 1),
    )

    remaining = set(range(1, P24_C_DEGREE))
    orbits: list[set[int]] = []
    while remaining:
        start = min(remaining)
        orbit = set(orbit_under_multiplier(start, p_mod_c, P24_C_DEGREE))
        orbits.append(orbit)
        remaining.difference_update(orbit)

    one_orbit = set(orbit_under_multiplier(1, p_mod_c, P24_C_DEGREE))
    neg_one_orbit = set(
        orbit_under_multiplier(P24_C_DEGREE - 1, p_mod_c, P24_C_DEGREE)
    )
    inversion_swaps = {
        (-value) % P24_C_DEGREE for value in one_orbit
    } == neg_one_orbit
    real_pairs = {
        min(value, (-value) % P24_C_DEGREE)
        for value in range(1, P24_C_DEGREE)
    }
    real_remaining = set(real_pairs)
    real_orbit_sizes: list[int] = []
    while real_remaining:
        start_pair = min(real_remaining)
        start = start_pair
        orbit_pairs: set[int] = set()
        for value in orbit_under_multiplier(start, p_mod_c, P24_C_DEGREE):
            orbit_pairs.add(min(value, (-value) % P24_C_DEGREE))
        real_orbit_sizes.append(len(orbit_pairs))
        real_remaining.difference_update(orbit_pairs)

    return {
        "c_prime": P24_C_DEGREE,
        "p_mod_c": p_mod_c,
        "p_order_mod_c": c_order,
        "nonzero_c_character_count": P24_C_DEGREE - 1,
        "frobenius_c_orbit_count": len(orbits),
        "frobenius_c_orbit_size": max(len(orbit) for orbit in orbits),
        "all_c_orbits_have_size_89": int(
            all(len(orbit) == (P24_C_DEGREE - 1) // 2 for orbit in orbits)
        ),
        "minus_one_in_p_frobenius_subgroup_mod179": int(
            P24_C_DEGREE - 1 in one_orbit
        ),
        "inversion_swaps_two_half_products": int(inversion_swaps),
        "real_inversion_quotient_orbit_count": len(real_orbit_sizes),
        "real_inversion_quotient_orbit_size": max(real_orbit_sizes),
        "degree89_half_anchor_matches_kernel_degree": int(
            max(real_orbit_sizes) == (P24_C_DEGREE - 1) // 2
        ),
        "ordinary_cyclotomic_right_order": right_order,
        "ordinary_cyclotomic_visible_level_order": level_order,
        "ordinary_cyclotomic_visible_order_not_post_bc": int(
            level_order != P24_JACOBI_LEVEL
        ),
        "kl_supplies_r179_half_anchor_shape": 1,
        "kl_still_requires_unramified_artin_pullback": 1,
    }
...
def main() -> None:
    ...
    p24_kl_orbit_anchor = p24_kubert_lichtenbaum_orbit_anchor_checks()
    for key, value in p24_kl_orbit_anchor.items():
        print(f"p24_kubert_lichtenbaum_orbit_anchor_{key}={value}")
    ...
    print("  kubert_lichtenbaum_langlands_supplies_degree89_r179_anchor_shape=1")
    print("  kubert_lichtenbaum_visible_frobenius_still_not_p24_rho_quotient=1")
    ...
    if p24_anderson_shadow != {
        ...
        raise SystemExit(1)
    if p24_kl_orbit_anchor != {
        "c_prime": P24_C_DEGREE,
        "p_mod_c": 77,
        "p_order_mod_c": 89,
        "nonzero_c_character_count": P24_C_DEGREE - 1,
        "frobenius_c_orbit_count": 2,
        "frobenius_c_orbit_size": 89,
        "all_c_orbits_have_size_89": 1,
        "minus_one_in_p_frobenius_subgroup_mod179": 0,
        "inversion_swaps_two_half_products": 1,
        "real_inversion_quotient_orbit_count": 1,
        "real_inversion_quotient_orbit_size": 89,
        "degree89_half_anchor_matches_kernel_degree": 1,
        "ordinary_cyclotomic_right_order": 1,
        "ordinary_cyclotomic_visible_level_order": 89,
        "ordinary_cyclotomic_visible_order_not_post_bc": 1,
        "kl_supplies_r179_half_anchor_shape": 1,
        "kl_still_requires_unramified_artin_pullback": 1,
    }:
        raise SystemExit(1)
