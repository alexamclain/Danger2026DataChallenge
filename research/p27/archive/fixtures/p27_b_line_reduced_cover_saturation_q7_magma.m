// P27 B-line reduced_Unext cover q7 saturation-only Magma smoke.
//
// This is the first CAS tractability test for the reduced 4-u model:
//   (Unext - 2*x5)^2 = 4*(x5^2 + A*x5 + 1)
// written in the legal B-line source variables.

SetColumns(0);
q := 7;
F := GF(q);
A7<X,W,T,R,beta,Bline,Unext> := AffineSpace(F,7);
P := CoordinateRing(A7);
...
I := ideal<P | eq_E, eq_T, eq_compact, eq_Bline, eq_first_half, eq_reduced_Unext>;
bad := X*(X - 1)*(X + 1)*(T - 2*X^2)*(X^2 + 1)*U_den*A_den;
Isat := Saturation(I, bad);
S := Scheme(A7, Basis(Isat));

print "BREDUCED_SATURATION_ONLY", Dimension(S), #Basis(Isat);
print "RESULT p27_b_line_reduced_cover_saturation_q7 done";
