#!/usr/bin/env python3
"""PGL2 recurrence screen for the p27 B-line Kummer sequence.

The B-line descent shows that d3(B) and d4(B) are well-defined on the legal
B quotient.  This probe tests the cheap recurrence/coboundary shape:

    d4(B) = +/- d3((a*B + b)/(c*B + d)).

A full-coverage positive would be a real multi-gate coupling on P1_B.  A
negative result says the next B-line work should remain actual divisor/Kummer
class extraction rather than degree-one rational map searches.
"""
...
def run_field(q: int, keep_best: int) -> None:
    d3, d4, setup_stats = legal_b_maps(q)
    setup = Counter({f"setup_{key}": value for key, value in setup_stats.items()})
    setup["q_mod_16"] = q % 16
    setup["chi_minus_one"] = legendre(-1, q)
    setup["chi_two"] = legendre(2, q)
    setup["d3_B_rows"] = len(d3)
    setup["d3_B_plus"] = sum(1 for value in d3.values() if value == 1)
    setup["d3_B_minus"] = sum(1 for value in d3.values() if value == -1)
    setup["d4_B_rows"] = len(d4)
    setup["d4_B_plus"] = sum(1 for value in d4.values() if value == 1)
    setup["d4_B_minus"] = sum(1 for value in d4.values() if value == -1)
    print_counter(f"q{q}_setup", setup)

    stats, exact, best = find_pgl2_recurrences(q, d3, d4, keep_best)
    print_counter(f"q{q}_d3_to_d4_pgl2_stats", stats)
    print_hits(f"q{q}_d3_to_d4_exact_pgl2", exact)
    print_hits(f"q{q}_d3_to_d4_best_pgl2", best)
...
def main() -> int:
    ...
    print("p27 B-line PGL2 recurrence probe")
    print("screen = d4(B) = +/- d3((a*B+b)/(c*B+d))")
    print(f"small_primes = {args.small_primes}")
    for q in parse_ints(args.small_primes):
        run_field(q, args.keep_best)
    print("p27_b_line_pgl2_recurrence_probe_rows=1/1")
    return 0
