#!/usr/bin/env python3
"""Source-normalized phase-word screen for the p27 B-line V4 phases.

This is a bounded follow-up to the alpha/beta phase sequence probe.  It does
not fit arbitrary phase words.  It tests a pre-registered list of natural
V4-derived observables:

  alpha_j, beta_j
  alpha_j*alpha_{j+1}, alpha_j*beta_{j+1},
  beta_j*alpha_{j+1}, beta_j*beta_{j+1}
  cumulative alpha and beta products from gate 3

Each bucket is scored against a later all-plus selected target with the raw
source-draw denominator carried through the report.  Conditional lift alone is
not a promotion; the point is to catch or kill a source-normalized phase-word
lead before asking GPU to scale it.
"""
...
def observables(max_gate: int) -> list[Observable]:
    ...
    for gate in range(3, max_gate + 1):
        out.append(Observable(f"a{gate}", gate, single_value("alpha", gate)))
        out.append(Observable(f"b{gate}", gate, single_value("beta", gate)))
    for gate in range(3, max_gate):
        out.append(Observable(f"aa{gate}_{gate + 1}", gate + 1, link_value("alpha", gate, "alpha")))
        out.append(Observable(f"ab{gate}_{gate + 1}", gate + 1, link_value("alpha", gate, "beta")))
        out.append(Observable(f"ba{gate}_{gate + 1}", gate + 1, link_value("beta", gate, "alpha")))
        out.append(Observable(f"bb{gate}_{gate + 1}", gate + 1, link_value("beta", gate, "beta")))
    for gate in range(4, max_gate + 1):
        out.append(Observable(f"cuma3_{gate}", gate, cumulative_value("alpha", gate)))
        out.append(Observable(f"cumb3_{gate}", gate, cumulative_value("beta", gate)))
    return out
    ...
    for cand in dedupe_candidates(candidates):
        ...
        for gate in range(3, max_gate + 1):
            ...
            for x, prior in active:
                phase, phase_stats = phase_record(B, x, p, gate)
...
def survives_target(phases: tuple[Phase, ...], target_gate: int) -> bool:
    ...
    if len(phases) < target_len:
        return False
    return all(phase.product == 1 for phase in phases[:target_len])
...
def score_observable(
    ...
    if source_draws:
        ...
        stats["domain_per_source_x1000000000"] = stats["domain"] * 1_000_000_000 // source_draws
...
def screen_label(
    ...
    print(f"{label}_phase_word_table:")
    print(
...
def main() -> int:
    ...
    print("p27 B-line phase-word source-normalized probe")
    print("screen = pre-registered V4 phase observables against later all-plus target")
    ...
    print("p27_b_line_phase_word_source_probe_rows=1/1")
    return 0
