#!/usr/bin/env python3
"""Belyi-normalized lambda branch-divisor probe for p27 d3/d4.

The Kummer-line map has branch values K=0, K^2+4=0, infinity, so the natural
normalized coordinate is

    lambda = -K^2/4.

This probe tests whether the next all-plus bits become a genus <= 1 Belyi-line
source:

    z^2 = f(lambda), deg(f) <= 4,

where the branch divisor splits into rational linear and irreducible quadratic
factors over each guard field.  This is stronger than the earlier K-degree <=4
screen for lambda-shaped functions, since degree 4 in lambda has degree 8 in K.
"""
...
def to_lambda_rows(krows: list[KRow], q: int) -> tuple[list[LambdaRow], Counter]:
    by_lam: dict[int, int] = {}
    stats: Counter = Counter()
    inv4 = inv(4, q)
    for row in krows:
        lam = (-row.k * row.k * inv4) % q
        prior = by_lam.get(lam)
        if prior is not None:
            stats["lambda_collision"] += 1
            if prior != row.target:
                stats["mixed_lambda_class"] += 1
            continue
        by_lam[lam] = row.target
    out = [LambdaRow(lam=lam, target=target) for lam, target in sorted(by_lam.items())]
    stats["input_k_rows"] = len(krows)
    stats["lambda_rows"] = len(out)
...
def target_mask(rows: list[LambdaRow]) -> int:
    out = 0
...
def linear_atoms(rows: list[LambdaRow], q: int, table: list[int]) -> list[Atom]:
    values = [row.lam for row in rows]
    atoms: list[Atom] = []
    for a in range(q):
        mask = signature_for_values([(value - a) % q for value in values], table)
        if mask is None:
            continue
        atoms.append(Atom(1, mask, f"L-{a}"))
    return atoms


def irreducible_quadratic_atoms(rows: list[LambdaRow], q: int, table: list[int]) -> list[Atom]:
    values = [row.lam for row in rows]
    atoms: list[Atom] = []
    for b in range(q):
        b2 = b * b % q
        for c in range(q):
            ...
            mask = 0
            for i, value in enumerate(values):
                out = (value * value + b * value + c) % q
                chi = table[out]
                if chi == 0:
                    raise ValueError("irreducible lambda quadratic vanished on F_q row")
                if chi == -1:
                    mask |= 1 << i
            atoms.append(Atom(2, mask, f"L^2+{b}L+{c}"))
    return atoms
...
def factor_degree(desc: str) -> int:
    return 2 if desc.startswith("L^2") else 1
        ...
        for left in left_map.values():
            ...
            for polarity, desired in targets:
                ...
                for right_degree in range(max_right + 1):
                    ...
                    degree = sum(factor_degree(desc) for desc in descs)
...
def collect_rows(q: int) -> tuple[list[LambdaRow], list[LambdaRow], Counter]:
    ...
    ld3, d3_lambda_stats = to_lambda_rows(kd3, q)
    ld4, d4_lambda_stats = to_lambda_rows(kd4, q)
    stats: Counter = Counter()
    ...
    stats.update({f"d3_lambda_{key}": value for key, value in d3_lambda_stats.items()})
    stats.update({f"d4_lambda_{key}": value for key, value in d4_lambda_stats.items()})
    return ld3, ld4, stats
...
def run_target(label: str, rows: list[LambdaRow], q: int, max_degree: int, limit: int) -> None:
...
def main() -> int:
    ...
    print("p27 lambda-line branch-divisor probe")
    print("lambda = -K^2/4")
    print("factors = rational linear and irreducible quadratic over F_q")
    print(f"small_primes = {args.small_primes}")
    print(f"max_degree_lambda = {args.max_degree}")
    for q in parse_ints(args.small_primes):
        ld3, ld4, setup_stats = collect_rows(q)
        print(f"q={q}:")
        print_counter("  setup_stats", setup_stats)
        if "d3" in targets:
            run_target(f"q{q}_d3", ld3, q, args.max_degree, args.limit)
        if "d4" in targets:
            run_target(f"q{q}_d4", ld4, q, args.max_degree, args.limit)
    print("p27_lambda_branch_divisor_probe_rows=1/1")
    return 0
