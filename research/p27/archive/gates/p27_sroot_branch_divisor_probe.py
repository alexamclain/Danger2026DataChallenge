#!/usr/bin/env python3
"""Rational square-root K coordinate branch-divisor probe for p27.

On the 2-isogenous quotient
...
The lambda quotient K -> -K is not rational on the p27 doubled stratum, so the
next rational source coordinate is the square root

    S = (U^2 - 4)/(2V).

This probe tests whether d3/d4 become exact low-degree branch divisors in S:

    z^2 = f(S), deg(f) <= 4,

with branch support split into rational linear and irreducible quadratic
factors over the guard fields.
"""
...
def s_coordinate(row: QuotientRow, p: int) -> int | None:
    u = row.x % p
    v = row.w % p
    if v == 0:
        return None
    return (u * u - 4) % p * inv(2 * v, p) % p


def to_srows(rows: list[QuotientRow], p: int) -> tuple[list[SRow], Counter]:
    by_s: dict[int, int] = {}
    stats: Counter = Counter()
    for row in rows:
        s = s_coordinate(row, p)
        if s is None:
            stats["s_undefined_skip"] += 1
            continue
        prior = by_s.get(s)
        if prior is not None:
            stats["s_collision"] += 1
            if prior != row.target:
                stats["mixed_s_class"] += 1
            continue
        by_s[s] = row.target
    out = [SRow(s=s, target=target) for s, target in sorted(by_s.items())]
    stats["input_rows"] = len(rows)
    stats["s_rows"] = len(out)
    stats["plus"] = sum(1 for row in out if row.target == 1)
    stats["minus"] = sum(1 for row in out if row.target == -1)
    stats["paired_s_neg_s"] = sum(1 for row in out if (-row.s) % p in by_s)
    return out, stats
...
def target_mask(rows: list[SRow]) -> int:
    out = 0
...
def linear_atoms(rows: list[SRow], q: int, table: list[int]) -> list[Atom]:
    ...
    for a in range(q):
        ...
        if mask is None:
            continue
        atoms.append(Atom(1, mask, f"S-{a}"))
    return atoms


def irreducible_quadratic_atoms(rows: list[SRow], q: int, table: list[int]) -> list[Atom]:
    values = [row.s for row in rows]
    atoms: list[Atom] = []
    for b in range(q):
        b2 = b * b % q
        for c in range(q):
            ...
            for i, value in enumerate(values):
                ...
                chi = table[out]
                if chi == 0:
                    raise ValueError("irreducible S quadratic vanished on F_q row")
                if chi == -1:
                    mask |= 1 << i
            atoms.append(Atom(2, mask, f"S^2+{b}S+{c}"))
    return atoms
...
def factor_degree(desc: str) -> int:
    return 2 if desc.startswith("S^2") else 1
...
def collect_rows(q: int) -> tuple[list[SRow], list[SRow], Counter]:
...
def run_target(label: str, rows: list[SRow], q: int, max_degree: int, limit: int) -> None:
...
def main() -> int:
    ...
    print("p27 S-root branch-divisor probe")
    print("S = (U^2 - 4)/(2V), K = S^2")
    print("factors = rational linear and irreducible quadratic over F_q")
    print(f"small_primes = {args.small_primes}")
    print(f"max_degree_S = {args.max_degree}")
    for q in parse_ints(args.small_primes):
        sd3, sd4, setup_stats = collect_rows(q)
        print(f"q={q}:")
        print_counter("  setup_stats", setup_stats)
        if "d3" in targets:
            run_target(f"q{q}_d3", sd3, q, args.max_degree, args.limit)
        if "d4" in targets:
            run_target(f"q{q}_d4", sd4, q, args.max_degree, args.limit)
    print("p27_sroot_branch_divisor_probe_rows=1/1")
    return 0
