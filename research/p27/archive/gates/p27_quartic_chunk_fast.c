...
//
// Scan family:
//   chi(X^4 + aX^3 + bX^2 + cX + d) = polarity * sign_i
...
static void load_rows(const char *path) {
    ...
    for (int i = 0; i < row_count; i++) {
        ...
        int x4 = (int)((int64_t)x2 * x2 % q_value);
        rows[i] = (Row){.x = x, .sign = sign, .x2 = x2, .x3 = x3, .x4 = x4};
    }
    fclose(fp);
}
...
static void build_masks(void) {
    ...
    for (int d = 0; d < q_value; d++) {
        full_mask[d >> 6] |= UINT64_C(1) << (d & 63);
    }
    for (int offset = 0; offset < q_value; offset++) {
        for (int d = 0; d < q_value; d++) {
            int sign = chi_table[(d + offset) % q_value];
            if (sign == 0) {
                continue;
            }
            int idx = sign == 1 ? 1 : 0;
            masks[idx][offset][d >> 6] |= UINT64_C(1) << (d & 63);
        }
    }
}

static void decode_index(uint64_t index, int *a, int *b, int *c) {
    uint64_t q = (uint64_t)q_value;
    *a = (int)(index / (q * q));
    uint64_t rem = index - (uint64_t)(*a) * q * q;
    *b = (int)(rem / q);
    *c = (int)(rem - (uint64_t)(*b) * q);
}
...
int main(int argc, char **argv) {
    ...
    for (uint64_t index = start; index < end; index++) {
        ...
        for (int polarity_i = 0; polarity_i < 2; polarity_i++) {
            ...
            for (int r = 0; r < row_count; r++) {
                ...
                int offset = (int)(((int64_t)row.x4
                    + (int64_t)a * row.x3
                    + (int64_t)b * row.x2
                    + (int64_t)c * row.x) % q_value);
                bool any = false;
                for (int w = 0; w < word_count; w++) {
            ...
            exact_quartics += hit_count;
            polarity_hits[polarity_i] += hit_count;
            while (sample_count < sample_limit) {
                int d = first_bit_words(intersection);
                if (d < 0) {
                    break;
                }
                printf("hit_sample polarity=%d coeffs=%d,%d,%d,%d\n", polarity, a, b, c, d);
                intersection[d >> 6] &= ~(UINT64_C(1) << (d & 63));
                sample_count++;
            }
        }
    }
    ...
    double throughput = seconds > 0.0 ? (double)triples_scanned / seconds : 0.0;

    printf("p27 quartic fast chunk probe\n");
    printf("rows_file = %s\n", row_path);
    printf("field = %d\n", q_value);
    printf("rows = %d\n", row_count);
    printf("start = %" PRIu64 "\n", start);
    printf("end = %" PRIu64 "\n", end);
    printf("triples_scanned = %" PRIu64 "\n", triples_scanned);
    printf("polarity_-1_hits = %" PRIu64 "\n", polarity_hits[0]);
    printf("polarity_1_hits = %" PRIu64 "\n", polarity_hits[1]);
    printf("exact_quartics = %" PRIu64 "\n", exact_quartics);
    printf("elapsed_seconds = %.6f\n", seconds);
    printf("throughput_triples_per_second = %.3f\n", throughput);
    printf("p27_quartic_chunk_fast_rows=1/1\n");
    return 0;
}
