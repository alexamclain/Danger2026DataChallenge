#!/usr/bin/env python3
"""Verify a GPU-reported K-line monic quartic hit."""
...
DEFAULT_PACKET = (
    ...
    "p27_kline_quartic_targets_20260622.json"
)
...
def quartic_value(k_value: int, coeffs: tuple[int, int, int, int], p: int) -> int:
    a, b, c, d = coeffs
    K = k_value % p
    return (pow(K, 4, p) + a * pow(K, 3, p) + b * K * K + c * K + d) % p


def verify(target: dict[str, Any], coeffs: tuple[int, int, int, int], polarity: int) -> dict[str, int]:
    p = int(target["field"])
    stats = {
        "rows": 0,
        "matches": 0,
    ...
    for row in target["rows"]:
        ...
        if target_sign == 1:
            ...
            stats["minus_rows"] += 1
        chi = legendre(quartic_value(int(row["K"]), coeffs, p), p)
...
def main() -> int:
    ...
    print("p27 K-line quartic verifier")
    ...
    print("p27_kline_quartic_verify_rows=1/1")
    return 0
